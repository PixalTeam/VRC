...
final class SendWorker extends Thread {
    ...
    public SendWorker(Context context2, List<ReportSender> reportSenders2, boolean sendOnlySilentReports2, boolean approvePendingReports2) {
        this.context = context2;
        this.reportSenders = reportSenders2;
        this.sendOnlySilentReports = sendOnlySilentReports2;
        this.approvePendingReports = approvePendingReports2;
    }
    ...
    private void checkAndSendReports(Context context2, boolean sendOnlySilentReports2) {
        ...
        for (String curFileName : reportFiles) {
            if (!sendOnlySilentReports2 || this.fileNameParser.isSilent(curFileName)) {
                if (reportsSentCount >= 5) {
                    break;
                }
                Log.i(ACRA.LOG_TAG, "Sending file " + curFileName);
                try {
                    sendCrashReport(new CrashReportPersister(context2).load(curFileName));
                    deleteFile(context2, curFileName);
                    reportsSentCount++;
    ...
    private void deleteFile(Context context2, String fileName) {
        if (!context2.deleteFile(fileName)) {
