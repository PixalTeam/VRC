        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, Declaration.MODULE_REFERENCE);
                } else {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, Declaration.MODULE_REFERENCE);
                }
                if (lp.height == -2) {
                    childHeightSpec3 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec3 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Declaration.MODULE_REFERENCE);
                } else {
                    childHeightSpec3 = MeasureSpec.makeMeasureSpec(lp.height, Declaration.MODULE_REFERENCE);
                }
                ...
                if (child2.getVisibility() != 8) {
                    ...
                    if (child2.getVisibility() != 8) {
                        ...
                        if (!canSlide || child2 == this.mSlideableView) {
                            if (lp2.weight > 0.0f) {
                                if (lp2.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), Declaration.MODULE_REFERENCE);
                                } else if (lp2.height == -2) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                                } else if (lp2.height == -1) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Declaration.MODULE_REFERENCE);
                                } else {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp2.height, Declaration.MODULE_REFERENCE);
                                }
                                if (canSlide) {
                                    int newWidth = widthAvailable - (lp2.leftMargin + lp2.rightMargin);
                                    int childWidthSpec2 = MeasureSpec.makeMeasureSpec(newWidth, Declaration.MODULE_REFERENCE);
                                    if (measuredWidth != newWidth) {
                                        child2.measure(childWidthSpec2, childHeightSpec);
                                    }
                                } else {
                                    child2.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp2.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), Declaration.MODULE_REFERENCE), childHeightSpec);
                                }
                            }
                        } else if (lp2.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp2.weight > 0.0f)) {
                            if (!skippedFirstPass) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), Declaration.MODULE_REFERENCE);
                            } else if (lp2.height == -2) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                            } else if (lp2.height == -1) {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Declaration.MODULE_REFERENCE);
                            } else {
                                childHeightSpec2 = MeasureSpec.makeMeasureSpec(lp2.height, Declaration.MODULE_REFERENCE);
                            }
                            child2.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, Declaration.MODULE_REFERENCE), childHeightSpec2);
                        }
                    }
                }
            }
