...
public class Toolbar extends ViewGroup {
    ...
    private void measureChildConstrained(View child, int parentWidthSpec, int widthUsed, int parentHeightSpec, int heightUsed, int heightConstraint) {
        ...
        if (childHeightMode != 1073741824 && heightConstraint >= 0) {
            childHeightSpec = MeasureSpec.makeMeasureSpec(childHeightMode != 0 ? Math.min(MeasureSpec.getSize(childHeightSpec), heightConstraint) : heightConstraint, Declaration.MODULE_REFERENCE);
        }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int measuredWidth = View.resolveSizeAndState(Math.max(width3 + getPaddingLeft() + getPaddingRight(), getSuggestedMinimumWidth()), i7, -16777216 & childState);
        int i8 = heightMeasureSpec;
        int measuredHeight = View.resolveSizeAndState(Math.max(height2, getSuggestedMinimumHeight()), i8, childState << 16);
        if (shouldCollapse()) {
            measuredHeight = 0;
        }
