...
public class testing extends ModuleBody {
    ...
    public static Object $PcTestShouldExecute(Object runner) {
        int i;
        try {
            Object run = $PcTestRunnerRunList((test$Mnrunner) runner);
            if (run == Boolean.TRUE) {
                i = 1;
            } else {
                i = 0;
            ...
            if (i == 0) {
                Object $PcTestAnySpecifierMatches = $PcTestAnySpecifierMatches(run, runner);
                try {
                    i = $PcTestAnySpecifierMatches != Boolean.FALSE ? 1 : 0;
                } catch (ClassCastException e) {
                    throw new WrongType(e, "x", -2, $PcTestAnySpecifierMatches);
                }
            }
            boolean x = (i + 1) & true;
            if (!x) {
                try {
                    ...
                    throw new WrongType(e2, "%test-runner-skip-list", 0, runner);
                }
            }
            testResultSet$Ex(runner, Lit1, Lit2);
            return Boolean.FALSE;
        } catch (ClassCastException e3) {
            throw new WrongType(e3, "%test-runner-run-list", 0, runner);
        }
    }
