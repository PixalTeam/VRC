...
public class CompileMisc implements Inlineable {
    ...
    public static Expression validateApplyMap(ApplyExp exp, InlineCalls visitor, Type required, Procedure xproc) {
        ...
        for (int i5 = 0; i5 < nargs2; i5++) {
            doArgs[i5 + 1] = visitor.visitApplyOnly(SlotGet.makeGetField(new ReferenceExp(pargs[i5]), "car"), null);
            recArgs[i5] = visitor.visitApplyOnly(SlotGet.makeGetField(new ReferenceExp(pargs[i5]), "cdr"), null);
        }
        if (!procSafeForMultipleEvaluation) {
            proc2 = new ReferenceExp(procDecl);
        }
        doArgs[0] = proc2;
        ApplyExp applyExp = new ApplyExp((Expression) new ReferenceExp(mproc.applyFieldDecl), doArgs);
        Expression doit = visitor.visitApplyOnly(applyExp, null);
        if (collect) {
            ReferenceExp referenceExp = new ReferenceExp(resultDecl);
            recArgs[nargs2] = Invoke.makeInvokeStatic(Compilation.typePair, "make", new Expression[]{doit, referenceExp});
        }
        ReferenceExp referenceExp2 = new ReferenceExp(loopDecl);
        ApplyExp applyExp2 = new ApplyExp((Expression) referenceExp2, recArgs);
        ? visitApplyOnly = visitor.visitApplyOnly(applyExp2, null);
        if (!collect) {
            BeginExp beginExp = new BeginExp(doit, visitApplyOnly);
            visitApplyOnly = beginExp;
        }
        lambdaExp.body = visitApplyOnly;
        ...
        while (true) {
            ...
            Expression[] compArgs = {new ReferenceExp(largs[i6]), empty};
            Expression result = collect ? new ReferenceExp(resultDecl) : QuoteExp.voidExp;
            ApplyExp applyExp3 = new ApplyExp((Procedure) mproc.isEq, compArgs);
            IfExp ifExp = new IfExp(visitor.visitApplyOnly(applyExp3, null), result, lambdaExp.body);
        ...
        ReferenceExp referenceExp3 = new ReferenceExp(loopDecl);
        ApplyExp applyExp4 = new ApplyExp((Expression) referenceExp3, initArgs);
        Expression body = visitor.visitApplyOnly(applyExp4, null);
        if (collect) {
