...
class LispTabulateFormat extends ReportFormat {
    ...
    public int format(Object[] args, int start, Writer dst, FieldPosition fpos) throws IOException {
        ...
        int spaces = column >= 0 ? !this.relative ? column < colnum2 ? colnum2 - column : colinc2 <= 0 ? 0 : colinc2 - ((column - colnum2) % colinc2) : (colnum2 + colinc2) - ((column + colnum2) % colinc2) : this.relative ? colnum2 : 2;
        while (true) {
            spaces--;
            if (spaces < 0) {
                return start;
